...
async def get_applications(
    app_id: UUID, controller: AppController = Depends(get_app_controller)
):
    return await controller.get_application_by_id(app_id)


@router.post(
    "/register", response_model=AppCreateResponse, status_code=status.HTTP_201_CREATED
)
async def register_application(
    payload: AppCreate, controller: AppController = Depends(get_app_controller)
):
...
async def update_application(
    payload: AppUpdate, controller: AppController = Depends(get_app_controller)
):
